...
export default function AuditPage() {
  ...
  const fetchAllData = async () => {
    try {
      const [locationRes, quarantineRes, firmwareRes] = await Promise.all([
        fetch('/api/audit/location-changes'),
        fetch('/api/audit/quarantine-history'),
        fetch('/api/audit/firmware-logs')
      ]);
  ...
  const fetchDeviceSpecificData = async (deviceId) => {
    try {
      const [locationRes, quarantineRes, firmwareRes] = await Promise.all([
        fetch(`/api/audit/location-changes?deviceId=${deviceId}`),
        fetch(`/api/audit/quarantine-history?deviceId=${deviceId}`),
        fetch(`/api/audit/firmware-logs?deviceId=${deviceId}`)
      ]);
